...
- [x] 2.3c **Vá thêm 2 góp ý Minor của CodeRabbit ở lần review thứ 2 (24/08/2026)** trên chính bản vá 2.3b: (1) chỉ kiểm tra "truthy" không chặn được `path`/`name` kiểu không phải string (số/boolean) — `.startsWith()` trên non-string sẽ throw ra lỗi 500 thay vì 400 gọn — đã ép rõ `typeof "string"`; (2) `/api/uploads` không chặn file 0 byte trong khi route này lại chặn (`size <= 0`), tạo ra tình huống tải lên thành công nhưng không đính kèm được — đổi thành `size < 0` để 2 route thống nhất cùng 1 quy tắc.
