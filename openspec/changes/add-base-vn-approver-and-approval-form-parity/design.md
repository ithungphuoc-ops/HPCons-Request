...
3. ~~"Cho phép người duyệt chuyển quyền duyệt cho người khác" có phải cùng khái niệm với "Chuyển tiếp" (ForwardModal) đã có?~~ — **ĐÃ CHỐT 24/08/2026: ĐÚNG, cùng khái niệm** — khớp chính xác "Chuyển tiếp và Duyệt" (`forward_then_approve`) đã có sẵn. Đã nối cờ này vào đúng chỗ (ẩn/hiện UI + chặn server), đổi mặc định thành `true` (giữ hành vi cũ), xem change `add-real-email-notifications-and-delegate-approval`.
...
5. ~~Thời hạn xử lý riêng bước có cần TÍNH LẠI mỗi khi đề xuất chuyển sang bước duyệt tiếp theo không~~ — **ĐÃ CHỐT 23/08/2026: có, cần tính lại.** Triển khai ở change riêng `recompute-approver-step-deadline` (xem design.md của change đó) — `resolveInitialSlaHours()` ở đây vẫn giữ nguyên, chỉ dùng cho lần tính LÚC GỬI đầu tiên.
